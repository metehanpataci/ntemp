...
uses
  ...
  Observer, MQTTClient, OverbyteIcsMQTT, NTempData;
...
type
  TMainForm = class(TForm, IObserver)
    ...
    procedure ClearLabelClick(Sender: TObject);
...
procedure TMainForm.ClearLabelClick(Sender: TObject);
begin
...
begin
  ...
  if MessagesStringGrid.ColCount >= 2 then
    MessagesStringGrid.ColWidths[2] := Self.Width -
      (MessagesStringGrid.ColWidths[0] + MessagesStringGrid.ColWidths[1]) - 10;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      data := TNTempData.TestInstance;
      FMQTTCl.Publish('ntemptest', data.ToJSON, qtAT_MOST_ONCE, false);
      data.Destroy;
    end
  end
  else
